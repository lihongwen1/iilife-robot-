...
public class MapActivity_A7_ extends BaseActivity implements View.OnClickListener {
    final String TAG = MapActivity_A7_.class.getSimpleName();
    public static final int WHAT = 110;
    ...
    int position = 0;
    byte sendByte;
    boolean isWork, hasAppoint, isMaxMode, hasStart, registerTag, hasStart_, voiceOpen, has_start_clean, isContinue;
    ...
    Animation animation, animation_alpha, animation_alpha_;
    ImageView image_quan, image_circle1, image_circle2, image_circle3;
    View layout_recharge, layout_key_point, layout_along, layout_clean;
...
//    private IntentFilter filter;
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if ((curStatus == 0x06 || curStatus == 0x03) && isContinue) {
                setAlphaAnimation();
            }
        }
    };
...
//    }
    ...
    public void setAlphaAnimation() {
        MyLog.e(TAG, "setAlphaAnimation position = " + position);
        imageViews.get(position).setVisibility(View.VISIBLE);
        imageViews.get(position).startAnimation(animation_alpha_);
        curImageView = imageViews.get(position);
        position++;
...
//        }
    ...
    private void initView() {
        ...
        layout_clean = findViewById(R.id.layout_clean);
...
//        canvasView.setVisibility(View.GONE);
    ...
    public void initData() {
        ...
        image_circle1 = findViewById(R.id.image_circle1);
        image_circle2 = findViewById(R.id.image_circle2);
        image_circle3 = findViewById(R.id.image_circle3);
        imageViews = new ArrayList<>();
        imageViews.add(image_circle1);
        imageViews.add(image_circle2);
        imageViews.add(image_circle3);
        ...
        animation_alpha_ = AnimationUtils.loadAnimation(context, R.anim.anim_alpha_);
        ...
        propertyReceiver = new ACDeviceDataMgr.PropertyReceiver() {
            ...
            public void onPropertyReceive(String s, long l, String s1) {
                ...
                if (!isDestroyed()) {
                    ...
                    voiceOpen = info.getVoice_mode() == 0x00;
...
//    }
    ...
    public void initMenuView(View contentView) {
        ...
        image_along.setVisibility(View.GONE);
        tv_along.setVisibility(View.GONE);
    ...
    public void setTextStatus(int tag) {
        switch (tag) {
            ...
            case TAG_CLEAN:
                tv_use_control.setVisibility(View.VISIBLE);
                tv_use_control.setText(getString(R.string.map_aty_clean));
                break;
        }
    }
...
//                            setTimeAndArea(curStatus);
                    ...
                    @Override
                    public void error(ACException e) {
                        MyLog.e(TAG, "sendToDeviceWithOption e " + e.toString());
                    }
                });
    }
    ...
    public void setStatus(int curStatus, int batteryNo, int mopForce, boolean isMaxMode, boolean voiceOpen) {
        ...
        } else if (curStatus == 0x0A) {
            setTextStatus(TAG_CONTROL);
        } else if (curStatus == 0x02 ||
                curStatus == 0x0C) {
        ...
        } else if (curStatus == 0x06 || curStatus == 0x03) {
            if (!has_start_clean) {
                position = 0;
                isContinue = true;
                layout_clean.setVisibility(View.VISIBLE);
                image_circle1.setVisibility(View.GONE);
                image_circle2.setVisibility(View.GONE);
                image_circle3.setVisibility(View.GONE);
                setAlphaAnimation();
                image_ele.bringToFront();
                has_start_clean = true;
            }
            setTextStatus(TAG_CLEAN);
        }
    ...
    public void clearAll(int curStatus) {
        ...
        if (curStatus != 0x06 && curStatus != 0x03) {
//            image_circle1.clearAnimation();
//            image_circle2.clearAnimation();
//            image_circle3.clearAnimation();
            has_start_clean = false;
...
////        canvasView.setVisibility(View.GONE);
//        layout_clean.setVisibility(View.GONE);
        if (controlPopup == null || !controlPopup.isShowing()) {
            tv_use_control.setVisibility(View.GONE);
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.image_start:  //done
                ...
                if (!isWork(curStatus)) {
                    int workMode = SpUtils.getInt(context, physicalId + SettingActivity.KEY_MODE);
                    if (workMode == 0x03 || workMode == 100) {
            ...
            case R.id.image_setting:
                Intent i = new Intent(context, SettingActivity_a7.class);
                startActivity(i);
                break;
            case R.id.image_clock:
                ...
                break;
            case R.id.image_along:
                if (curStatus == 0x04 || curStatus == 0x02) {
                    mAcDevMsg.setCode(MsgCodeUtils.WorkMode);
